...
//     $price_q = $conn->query("SELECT * FROM props ORDER BY price_sort $price");
//     $price_q->execute();
//     $allListings = $price_q->fetchAll(PDO::FETCH_OBJ);
//   }

if(isset($_GET['name'])){
    $hometype=$_GET['name'];

    $homes = $conn->query("SELECT * FROM props WHERE home_type = '$hometype'");
    $homes->execute();
    $allListings = $homes->fetchAll(PDO::FETCH_OBJ);
  }else{
            ...
            <div class="property-entry h-100">
              <a href="property-details.php?id=<?php echo $allListing->id; ?>" class="property-thumbnail">
                <div class="offer-type-wrap">
                  <span class="offer-type bg-<?php echo ($allListing->type == 'rent')?"success":"danger"; ?>"><?php echo $allListing->type;?></span>
                </div>
                <img src="<?php echo APPURL; ?>/images/<?php echo $allListing->image; ?>" alt="Image" class="img-fluid">
              </a>
